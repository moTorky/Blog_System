...
session_start();
$errors = array();
function get_selected_categories($post_id){
    global $db;
    $sql=$db->prepare('SELECT category_id FROM `post_category` WHERE `post_id`='.$post_id);
    $sql->execute();
    $categories=$sql->fetchAll(PDO::FETCH_COLUMN);
    return $categories;

}
function get_selected_images($post_id){
    global $db;
...
if(empty($_SESSION['id'])){
    ...
    if(empty($_POST['post_id'])){
        $errors['edite_id_err'] = 'post id not set.';
    ...
    $sql=$db->prepare('SELECT * FROM `posts` WHERE `id`='.$post_id);
    $sql->execute();
    $post=$sql->fetch(PDO::FETCH_ASSOC);
    $old_categories=get_selected_categories($post_id);



    //get image list
    if (!empty($_FILES['images']['name'][0])) {
        $image_paths = get_selected_images($post_id); // Array to store image paths
                    
        foreach ($_FILES['images']['name'] as $index => $filename) {
            $temp_file = $_FILES['images']['tmp_name'][$index];
...
?>
	<div class="site-cover site-cover-sm same-height overlay single-page" style="background-image: url('<?php echo $post['cover_img_path'];?>');">
		<div class="container">
			<div class="row align-items-center justify-content-center text-center pt-5">
				<div class="col-lg-6">
					<h1 class="heading text-white mb-3" data-aos="fade-up">Edit Post</h1>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="row">
                    ...
                    <?php
                        if ( !empty($image_paths)) {
                            ...
                            foreach ($image_paths as $img_path) {
                                ...
                                echo '<li>' . basename($img_path) . '</li>'; // Display the image file name
                            }
                            echo '</ul>';
                        }
                    ?>
				...
				<div class="col-lg-8" data-aos="fade-up" data-aos-delay="200">
                    <form method="post" action="" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-12 mb-3">
                                    <input type="text" name="title" class="form-control" placeholder="post title" value=<?php echo $post['title'];?>>
                                ...
                                <div class="col-6 mb-3">
                                    ...
                                    <div class="dropdown" name="categories">
                                        <?php foreach($categories as $category){?>
                                            <input type="checkbox" value=<?php echo $category['id'].' ';?> name="categories[]" <?php if(in_array($category['id'],$old_categories)) echo 'checked';?>>
                                ...
                                <div class="col-12 mb-3">
                                    <textarea name="content" id="" cols="30" rows="7" class="form-control" placeholder="Contnet"><?php echo $post['content'];?></textarea>
